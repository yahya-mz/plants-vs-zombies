...
import com.pvz.plantsvszombies.Presentation.Animations.Zombies.ConeHeadZombieAnimations;
...
public class ConeHeadZombieVisualObject extends AbstractZombieVisualObject {
    ...
    public ConeHeadZombieVisualObject changeStateTo(AbstractZombieGameObject.GeneralZombieState state) {
        Platform.runLater(()->{
            switch (state) {
                case MOVING_FORWARD -> {
                    _currentVisualState = AbstractZombieGameObject.GeneralZombieState.MOVING_FORWARD;
                    playAnimation(ConeHeadZombieAnimations.Animations.MOVING_FORWARD, Duration.millis(30));
                }
                case FREEZY_MOVING_FORWARD -> {
                    _currentVisualState = AbstractZombieGameObject.GeneralZombieState.FREEZY_MOVING_FORWARD;
                    playAnimation(ConeHeadZombieAnimations.Animations.FROZEN_MOVING_FORWARD, Duration.millis(60));
                }
                case DYING -> {
                    _currentVisualState = AbstractZombieGameObject.GeneralZombieState.DYING;
                    playAnimation(ConeHeadZombieAnimations.Animations.DYING, Duration.millis(60), 1);
                ...
                case EATING -> {
                    ...
                    playAnimation(ConeHeadZombieAnimations.Animations.ATTACKING, Duration.millis(30));
                }
                case FREEZY_EATING -> {
                    _currentVisualState = AbstractZombieGameObject.GeneralZombieState.FREEZY_EATING;
                    playAnimation(ConeHeadZombieAnimations.Animations.FROZEN_ATTACKING, Duration.millis(60));
                }
                case BURNING -> {
