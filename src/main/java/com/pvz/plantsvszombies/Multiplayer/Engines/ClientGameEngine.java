...
public class ClientGameEngine extends GameEngine {
    ...
    public void update() {
        ...
        if (tick % (GlobalSettings.FPS * 5) == 0) { // Every 5 seconds
            sendClientStatus(ClientStatusEvent.ClientStatus.PLAYING);
        }
    ...
    private void checkWinCondition() {
        ...
        if (_currentWave >= 4) {
            ...
            if (!hasZombies && System.currentTimeMillis() - _gameStartTime > 90000) { // After 90 seconds
                sendClientStatus(ClientStatusEvent.ClientStatus.WON);
                System.out.println("🎉 You completed all waves! Waiting for server confirmation...");
            }
        }
    }

    private void sendClientStatus(ClientStatusEvent.ClientStatus status) {
    ...
    public void disconnect() {
        sendClientStatus(ClientStatusEvent.ClientStatus.DISCONNECTED);
